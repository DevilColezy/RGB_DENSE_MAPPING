...
public:
	...
	void addActiveFrame(ImageAndExposure* image, int id, cv::Mat img_bgr);
	...
	SE3 getCurPose();
	int getWindowSize(){
		return frameHessians.size();
	}

	bool haveNewKeyFrame;
	//FrameShell* centerFrame;
	std::queue<FrameHessian*> slidewindows;
