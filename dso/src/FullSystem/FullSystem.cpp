...
SE3 FullSystem::getCurPose(){
	return allFrameHistory[allFrameHistory.size()-1]->camToWorld;
}
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id, cv::Mat img_bgr)
{
    if(isLost) return;
	boost::unique_lock<boost::mutex> lock(trackMutex);
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	while(!slidewindows.empty()){
		slidewindows.pop();
	}

	double x_ctr, y_ctr, z_ctr;
	for(unsigned int i=0; i<frameHessians.size(); i++){
		x_ctr += frameHessians[i]->shell -> camToWorld.matrix()(0,3);
		y_ctr += frameHessians[i]->shell -> camToWorld.matrix()(1,3);
		z_ctr += frameHessians[i]->shell -> camToWorld.matrix()(2,3);
	}
	x_ctr /=frameHessians.size();
	y_ctr /=frameHessians.size();
	z_ctr /=frameHessians.size();

	double d_ctr = DBL_MAX;
	double d_tmp;
	int i_ctr;

	for(unsigned int i=0; i<frameHessians.size(); i++){
		d_tmp = 0;
		d_tmp += (frameHessians[i] -> shell ->camToWorld.matrix()(0,3) -x_ctr) * (frameHessians[i] -> shell ->camToWorld.matrix()(0,3) -x_ctr);
		d_tmp += (frameHessians[i] -> shell ->camToWorld.matrix()(1,3) -y_ctr) * (frameHessians[i] -> shell ->camToWorld.matrix()(1,3) -y_ctr);
		d_tmp += (frameHessians[i] -> shell ->camToWorld.matrix()(2,3) -z_ctr) * (frameHessians[i] -> shell ->camToWorld.matrix()(2,3) -z_ctr);
		if(d_tmp < d_ctr){
			d_ctr = d_tmp;
			i_ctr = i;
		}
	...
	slidewindows.push(frameHessians[i_ctr]);

	for(unsigned int i=0; i<frameHessians.size(); i++){
		if(i == i_ctr){
			continue;
		}
		slidewindows.push(frameHessians[i]);
	}
